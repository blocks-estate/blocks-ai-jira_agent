...
import type { KpiMetric, FunnelStage, WeeklyReviewSection, ActionItem } from '../../api'
...
function Overview() {
  ...
  useEffect(() => { fetchJSON<any>('/api/overview').then(setData) }, [])

  if (!data) return <div className="loading">Loading dashboard overview...</div>
...
function DeveloperDashboard() {
  ...
  useEffect(() => { fetchJSON<any>('/api/kpis/developer').then(setData) }, [])
...
function InvestorDashboard() {
  const [data, setData] = useState<any>(null)
  const [period, setPeriod] = useState<'90days' | '12months'>('90days')
  useEffect(() => { fetchJSON<any>('/api/kpis/investor').then(setData) }, [])
...
function PrDashboard() {
  const [data, setData] = useState<any>(null)
  useEffect(() => { fetchJSON<any>('/api/kpis/pr').then(setData) }, [])
...
function SeoDashboard() {
  const [data, setData] = useState<any>(null)
  useEffect(() => { fetchJSON<any>('/api/kpis/seo-content').then(setData) }, [])
...
function TrustComplianceDashboard() {
  const [data, setData] = useState<any>(null)
  useEffect(() => { fetchJSON<any>('/api/kpis/trust-compliance').then(setData) }, [])

  if (!data) return <div className="loading">Loading Trust/Compliance KPIs...</div>

  return (
    <div>
      <div className="page-header"><h1>Trust & Compliance KPI Dashboard</h1><p>{data.description}</p></div>
      <div className="grid-4">{data.metrics.map((m: KpiMetric) => <KpiCard key={m.id} metric={m} />)}</div>
      <KpiList metrics={data.metrics} />
      {data.metrics.map((m: KpiMetric) => (
        <div key={m.id + '-chart'} className="chart-container">
...
function FunnelsView() {
  ...
  useEffect(() => {
    fetchJSON<any>('/api/funnels/investor').then(setInvestor)
    fetchJSON<any>('/api/funnels/developer').then(setDeveloper)
  }, [])
    ...
    <div>
      ...
      <div className="grid-4">
        <div className="stat-card">
          ...
          <div className="stat-target">From awareness to repeat investment</div>
        </div>
        <div className="stat-card">
          ...
          <div className="stat-target">From target account to asset approved</div>
        </div>
        <div className="stat-card">
...
function ShareOfVoice() {
  const [data, setData] = useState<any>(null)
  useEffect(() => { fetchJSON<any>('/api/share-of-voice').then(setData) }, [])
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Competitive visibility tracking with channel/scenario taxonomy</p>
        <div className="meta-bar">
...
function SubscriptionTracking() {
  const [data, setData] = useState<any>(null)
  useEffect(() => { fetchJSON<any>('/api/subscription-tracking').then(setData) }, [])
...
function KpiDictionary() {
  ...
  useEffect(() => { fetchJSON<any>('/api/kpi-dictionary').then(setData) }, [])
...
function Instrumentation() {
  ...
  useEffect(() => { fetchJSON<any>('/api/funnel-instrumentation').then(setData) }, [])
  ...
  return (
    <div>
      <div className="page-header"><h1>Funnel Instrumentation Spec</h1><p>{data[tab].description}</p></div>
  ...
  return (
    <div>
      <div className="tabs" style={{ marginBottom: 20 }}>
        ...
        <button className={`tab ${view === 'developer' ? 'active' : ''}`} onClick={() => setView('developer')}>Developer KPIs</button>
        <button className={`tab ${view === 'investor' ? 'active' : ''}`} onClick={() => setView('investor')}>Investor KPIs</button>
        <button className={`tab ${view === 'pr' ? 'active' : ''}`} onClick={() => setView('pr')}>PR KPIs</button>
        <button className={`tab ${view === 'seo' ? 'active' : ''}`} onClick={() => setView('seo')}>SEO/Content</button>
        <button className={`tab ${view === 'trust-compliance' ? 'active' : ''}`} onClick={() => setView('trust-compliance')}>Trust/Compliance</button>
        <button className={`tab ${view === 'funnels' ? 'active' : ''}`} onClick={() => setView('funnels')}>Funnels</button>
        <button className={`tab ${view === 'share-of-voice' ? 'active' : ''}`} onClick={() => setView('share-of-voice')}>Share of Voice</button>
        <button className={`tab ${view === 'docs' ? 'active' : ''}`} onClick={() => setView('docs')}>Documents</button>
        <button className={`tab ${view === 'subscriptions' ? 'active' : ''}`} onClick={() => setView('subscriptions')}>Subscriptions</button>
        <button className={`tab ${view === 'weekly-review' ? 'active' : ''}`} onClick={() => setView('weekly-review')}>Review</button>
        <button className={`tab ${view === 'dictionary' ? 'active' : ''}`} onClick={() => setView('dictionary')}>Dictionary</button>
        <button className={`tab ${view === 'instrumentation' ? 'active' : ''}`} onClick={() => setView('instrumentation')}>Instrumentation</button>
      </div>
